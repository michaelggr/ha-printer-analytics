...
# 替换旧的循环代码为新的代码
old_code = '''  _renderRealtimeMonitor() {
...
new_code = '''  _renderRealtimeMonitor() {
    const printers = this._getPrintersConfig();
    let html = `<div class="section-header"><div class="section-title"><span class="section-icon">📡</span><span>实时监控面板</span></div></div>`;
...
if old_code in content:
    ...
    print("Replaced realtime monitor code")
else:
    print("Code not found, trying with etaTime to endTime...")
    # 可能变量名不同，尝试修改
    old_code_fixed = old_code.replace('const etaTime =', 'const endTime =').replace('if (etaTime', 'if (endTime').replace('const etaDate', 'const endDate')
    if old_code_fixed in content:
        content = content.replace(old_code_fixed, new_code)
        print("Replaced with endTime version")
...
print("Modified realtime monitor to show selected printer only")
