...
config = config.replace(old_temp, new_temp)
...
config = config.replace(old_hum, new_hum)

# 5. 找到首页视图的起始和结束
# 从首页视图开始到功耗统计视图开始
home_start = config.find('  - title: 首页')
power_start = config.find('  - title: 功耗统计')

# 提取首页视图内容
home_content = config[home_start:power_start]
...
# 精确的替换
old_end_part = '''        align_header: default
        align_icon: right
...
new_end_part = '''        align_header: default
        align_icon: right
...
config = config.replace(old_end_part, new_end_part)
