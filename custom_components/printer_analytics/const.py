...
SERVICE_BACKFILL_TASK_NAMES_FROM_HISTORY = "backfill_task_names_from_history"
SERVICE_BACKFILL_RECORD_FIELDS = "backfill_record_fields"
SERVICE_IMPORT_HISTORY = "import_history"
SERVICE_BACKUP_HISTORY = "backup_history"
SERVICE_UPDATE_RECORD_FIELD = "update_record_field"
...
# 结束打印状态（保存记录，含别名兼容）
END_PRINT_STATUSES = {PRINT_STATUS_FINISH, PRINT_STATUS_FAIL, PRINT_STATUS_FAILED, PRINT_STATUS_CANCELLED}
# 成功状态集合（含中英文/导入数据变体）
SUCCESS_STATUSES = {PRINT_STATUS_FINISH, "completed", "success", "完成", "成功"}
# 失败状态集合（含 Bambu 原始值 "failed" 和中文）
FAILURE_STATUSES = {PRINT_STATUS_FAIL, PRINT_STATUS_FAILED, "失败"}
# 取消状态集合
CANCELLED_STATUSES = {PRINT_STATUS_CANCELLED, "已取消"}
# 中间过渡状态（不触发开始/结束）
TRANSITIONAL_STATUSES = {"prepare", "slicing", "init"}
# 离线状态
