...
- **摄像头平滑切换** - 点击按钮展开/收起摄像头视图，带过渡动画，按钮状态实时反馈
- **多 AMS 支持** - 支持 AMS1~4（最多16托盘），多 AMS 时显示编号分隔
- **手机端跑马灯** - 任务名在窄屏自动滚动，两端渐隐+起止暂停
- **Bambu Cloud 同步** - 手机号+验证码登录，一键同步云端打印历史，每12小时自动拉取
...
- **功能**：V2 紧凑布局设为默认监控面板，移除旧版布局代码
- **功能**：多 AMS 支持 — AMS1~4（最多16托盘），多 AMS 时显示编号分隔
- **功能**：摄像头按钮改为文字按钮「📷 摄像头」，打开时按钮亮起
- **功能**：摄像头视图打开/关闭过渡动画（0.4秒平滑展开/收起）
- **功能**：任务名手机端跑马灯 — CSS 动画 + 两端渐隐 + 起止暂停
- **功能**：摄像头按钮 toggle 行为 — 再次点击可关闭
- **修复**：统计分析页打印记录点击无法弹出详情
- **修复**：统计分析页无限刷新循环
- **修复**：Bambu Cloud API Content-Type 兼容性
- **修复**：check_token 网络异常时不清除 Token
